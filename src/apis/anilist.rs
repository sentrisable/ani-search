...
fn post_anilist(payload: &String, token: &str) -> Result<String, Box<dyn std::error::Error>>{
...
pub fn get_anime_from_list(user_id: i64, token: &str)->Result<anilist_user_shows::MediaListCollection, Box<dyn std::error::Error>>{ 
    let payload = json!({
...
pub fn search_anilist(show: &String, config: &Config) -> Result<anilist_search_shows::Page, Box<dyn std::error::Error>>{
    ...
    let response = post_anilist(&payload, &config.anilist.token)?;
...
pub fn update_progress(show_id: &i64, episode_number: Option<&String>, status: WatchStatus, config: &Config) -> Result<(), Box<dyn std::error::Error>>{
    dbg!(&show_id);
    dbg!(&episode_number);
    let payload = json!({
