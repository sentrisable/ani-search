...
use crate::{
    AniError, SearchProvider, SearchResult, Result, 
    RequestHeaders, SearchOptions, StreamLink, SubtitleTrack,
    Translation,
    anilist::DEFAULT_ANILIST_API,
...
impl AnikotoClient {
    ...
    pub async fn search(&self, query: &str, mode: Translation) -> Result<Vec<SearchResult>> {
    ...
    pub async fn episodes(&self, show_id: &str, _mode: Translation) -> Result<Vec<String>> {
...
pub fn provider_from_show_id(show_id: &str) -> SearchProvider {
        SearchProvider::Anikoto
}
    ...
    values
        ...
        .filter_map(|item| {
            ...
            Some(SearchResult {
                ...
                name: title,
                episodes,
                provider: SearchProvider::Anikoto,
            })
        })
        .collect()
}
...
fn embed_candidates(
    ...
    if let Some(selected) = selected {
        let explicit = match mode {
            Translation::Sub => selected.sub_url.as_ref(),
            Translation::Dub => selected.dub_url.as_ref(),
        };
...
//         assert_eq!(provider_from_show_id(&encoded), SearchProvider::Anikoto);
//         assert_eq!(
//             provider_from_show_id("anikoto2:metadata"),
//             SearchProvider::Anikoto2
//         );
//         assert_eq!(provider_from_show_id("legacy"), SearchProvider::Anikoto);
//     }
...
//             .unwrap();
//         let values = client
//             .search("example", Translation::Sub)
//             .await
//             .unwrap();
//         assert_eq!(values.len(), 1);
//         assert_eq!(values[0].provider, SearchProvider::Anikoto);
//         assert_eq!(
...
//             .unwrap()
//             .search("Frieren", Translation::Sub)
//             .await
//             .unwrap();
